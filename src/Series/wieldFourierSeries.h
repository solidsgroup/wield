...
public:
	...
	FourierSeries(int _order,
				  ...
				  std::vector<double> Z,
				  int numThreads = 1,
				  int verbose = false) : order(_order),
										 alphaX(_alphaX),
										 alphaY(_alphaY),
										 alphaZ(_alphaZ),
										 //sigma(_sigma)
			...
			for (int m = -order + 1; m < order; m++)
				for (int n = -order + 1; n < order; n++)
				{
					...
					for (unsigned int i = 0; i < size; i++)
					{
						...
						if (fabs(Y[i]) > 0.5 * alphaY)
							WIELD_WARNING("Y[i] = " << Y[i] << " > 0.5*alphaY= " << 0.5 * alphaY);
						if (fabs(Z[i]) > 0.5 * alphaZ)
							WIELD_WARNING("Z[i] = " << Z[i] << " > 0.5*alphaZ= " << 0.5 * alphaZ);

						double lambda = 1.;
						if (fabs(fabs(X[i]) - 0.5 * alphaX) < 1E-8)
							lambda *= 0.5;
						if (fabs(fabs(Y[i]) - 0.5 * alphaY) < 1E-8)
							lambda *= 0.5;
						if (fabs(fabs(Z[i]) - 0.5 * alphaZ) < 1E-8)
							lambda *= 0.5;
						...
						(*this)(l, m, n) +=
							lambda * phiHat(k) * exp(-I * (double)(k * x)) / (alphaX * alphaY * alphaZ);
					}
				}
		}
		if (verbose)
