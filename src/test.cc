...
complex<double> C(int l, int m, int n)
{
  ...
  if (fabs(imag(ret)) > 1E-5) cout << "Imaginary part detected --> =" << imag(ret) << endl;
  return ret/(alpha1*alpha2*alpha3);
}
Vector3d Alpha(int l, int m, int n)
{
  return Vector3d(2.*pi*l/alpha1,2.*pi*m/alpha2,2.*pi*n/alpha3);
}
}

double PhiHat(Vector3d k,double kT)
{
...
complex<double> VHat(Vector3d ka, Vector3d kb, double sigma, double epsilon)
{
  complex<double> I(0,1.);
  // double sigma = 1.;
  // double epsilon = 0.1;
	 ...
	 char* argv[]) 
{
  ...
  Series C(order);
  //vector<complex<double> > C((2*order+1)*(2*order+1)*(2*order+1));
	    ...
	    for (int l=-order;l<=order;l++)
	      for (int m=-order;m<=order;m++)
	    	for (int n=-order;n<=order;n++)
	    	  val += C(l,m,n)*exp(complex<double>(0.,FCC::Alpha(l,m,n).dot(Vector3d(x,y,z))));
	    //val = 2*x;// exp(complex<double>(0,2*pi*x/FCC::alpha1));
	    float v = real(val);
	    //cout << v << endl;
  ...
  for (double theta=0.;theta<=180.;theta+=0.5)
    {
      ...
      //Matrix3d R = createMatrixFromZX(Vector3d(0,0,1),Vector3d(1,0,0));
      Matrix3d R = createMatrixFromZX(Vector3d(1,-1,0),Vector3d(1,1,0));
      //Matrix3d R = createMatrixFromZX(Vector3d(1,0,-1),Vector3d(1,1,1));
      //Matrix3d R = createMatrixFromZX(Vector3d(1,-1,0),Vector3d(1,1,2));
      Matrix3d omega1 = createMatrixFromXAngle(+theta/2.)*R;
      Matrix3d omega2 = createMatrixFromXAngle(-theta/2.)*R;
      for (int i=-order; i<=order; i++)
	for (int j=-order; j<=order; j++)
	  for (int k=-order; k<=order; k++)
		...
		for (int m=-order; m<=order; m++)
		  for (int n=-order; n<=order; n++)
		    //if (l==0 && m==0 && n==0) continue; else
		      {
			complex<double> v = VHat(omega1*FCC::Alpha(i,j,k),omega2*FCC::Alpha(l,m,n),sigma,epsilon);
			complex<double> dw =
			  C(i,j,k) *
			  v *
			  C(l,m,n);
      ...
      out << theta << " " << w.real() << endl;
      cout << theta << " " << w.real() << endl;
    }
  out.close();
